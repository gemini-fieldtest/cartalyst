...
export const Analysis: React.FC = () => {
    ...
    const handleRunAnalysis = async () => {
        setIsAnalyzing(true);
        // Optimistic clear or keep old? Let's keep old while loading to avoid flicker if re-analyzing
        const result = await analyzeLap(selectedLap, idealLap, MOCK_TRACK);

        if (result) {
            setAnalysisMap(prev => ({
                ...prev,
                [selectedLapId]: result
            }));
        }
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {MOCK_SESSION.laps.filter(l => l.id !== 'lap_ideal').map(lap => (
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-lg transition-all border ${selectedLapId === lap.id
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                {/* Indicator if this lap has notes */}
                                {analysisMap[lap.id] && <Brain size={12} className="text-indigo-400" />}
                            </div>
            ...
            <div className="flex-1 flex flex-col overflow-y-auto p-6 md:p-8 gap-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-2.5 rounded font-bold transition-all shadow-lg text-sm ${isAnalyzing
